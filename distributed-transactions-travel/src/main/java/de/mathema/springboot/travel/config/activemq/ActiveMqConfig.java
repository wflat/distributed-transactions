...
public class ActiveMqConfig {
  ...
  @Bean
  @ConfigurationProperties(prefix = "travel.activemq")
  ...
  public JmsMessageSender hotelJmsMessageSender(final JmsTemplate jmsTemplate,
    ...
    return new JmsMessageSender(jmsTemplate, activeMqProperties.getHotel().getInputQueue());
  }

  @Bean
  public JmsMessageSender carJmsMessageSender(final JmsTemplate jmsTemplate,
                                              final ActiveMqProperties activeMqProperties) {
    return new JmsMessageSender(jmsTemplate, activeMqProperties.getCar().getInputQueue());
  }

  @Bean
  public JmsMessageSender flightJmsMessageSender(final JmsTemplate jmsTemplate,
                                                 final ActiveMqProperties activeMqProperties) {
    return new JmsMessageSender(jmsTemplate, activeMqProperties.getFlight().getInputQueue());
  }
